...
print('The current run contains the results of a small model for closed loop predictions')
...
class FC_small_model(nn.Module):
    ...
    def __init__(self, input_dim):
        ...
        self.fc1 = nn.Linear(in_features = input_dim, out_features = 256, bias = True)
        self.relu1 = nn.GELU()
        self.fc2 = nn.Linear(in_features = 256, out_features = 120, bias = True)
        self.relu2 = nn.GELU()
        self.fc3 = nn.Linear(in_features = 120, out_features = 2, bias = True)
...
model1 = lstm_small(2,50,n_layers = 2, do = 0.1)
num_params(model1)
model2 = FC_small_model(2)
num_params(model2)   
...
with open('/mnt/qb/work/goswami/ranganathabr08/common_directory/Mar_results/small_closed/meta/meta_data_small_lstm_fc.txt','w') as f:
...
for ensemble_index in range(num_ensembles):
    ...
    torch.save(error1_tensor,'/mnt/qb/work/goswami/ranganathabr08/common_directory/Mar_results/small_closed/errors/lstm_'+str(ensemble_index)+str(t)+'.pt')
    torch.save(model1, '/mnt/qb/work/goswami/ranganathabr08/common_directory/Mar_results/small_closed/models/lstm'+str(ensemble_index)+str(t)+'.pt')
    ...
    torch.save(error2_tensor,'/mnt/qb/work/goswami/ranganathabr08/common_directory/Mar_results/small_closed/errors/fc_'+str(ensemble_index)+str(t)+'.pt')
    torch.save(model2, '/mnt/qb/work/goswami/ranganathabr08/common_directory/Mar_results/small_closed/models/fc_'+str(ensemble_index)+str(t)+'.pt')
    trend1 = torch.mean(ls_tensor1, axis = 0)
    torch.save(trend1, '/mnt/qb/work/goswami/ranganathabr08/common_directory/Mar_results/small_closed/error_trend/lstm_'+str(ensemble_index)+str(t)+'.pt')
    trend2 = torch.mean(ls_tensor2, axis = 0)
    torch.save(trend2, '/mnt/qb/work/goswami/ranganathabr08/common_directory/Mar_results/small_closed/error_trend/fc_'+str(ensemble_index)+str(t)+'.pt')
