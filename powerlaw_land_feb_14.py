...
for year_index in range(1979,2020):
    ...
    data = pd.read_csv('/mnt/qb/work/goswami/ranganathabr08/cluster_files/cluster_files/land_clusters'+str(year_index)+'.csv',index_col = False, usecols = col_list)
    g = dg.DeepGraph(data)
...
plt.ylabel('p(S)')
plt.show()       
plt.savefig('/mnt/qb/work/goswami/ranganathabr08/common_directory/Feb_results/land_powerlaw_feb_14.jpg')            
        
file1 = open("/mnt/qb/work/goswami/ranganathabr08/common_directory/Feb_results/alphas.txt","a")
file1.write('land - alpha:'+str(data_fit1.alpha)+'\n')
file1.close()

vol_tensor = torch.Tensor(vol_list)
torch.save(vol_tensor, "/mnt/qb/work/goswami/ranganathabr08/common_directory/Feb_results/land_powerlaw_vol_list.pt")
