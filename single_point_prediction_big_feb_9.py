# This code trains point prediction models of LSTM and the DNN kind
...
for year_index in range(1979,1979+32):
	a = pd.read_csv('/mnt/qb/work/goswami/ranganathabr08/cluster_files/cluster_files/mix_clusters'+str(year_index)+'.csv',index_col = False)
	frame_list.append(a)
	
data = pd.concat(frame_list)
data = data.drop('Unnamed: 0', axis = 1)
train_data = data.drop('Unnamed: 0.1', axis = 1)
...
frame_list = []
for year_index in range(1979+32,2020):
	a = pd.read_csv('/mnt/qb/work/goswami/ranganathabr08/cluster_files/cluster_files/mix_clusters'+str(year_index)+'.csv',index_col = False)
	frame_list.append(a)
	
data = pd.concat(frame_list)
data = data.drop('Unnamed: 0',axis = 1)
test_data = data.drop('Unnamed: 0.1',axis= 1)

del a
del frame_list
del data

print('Testingdata has been loaded')

#=======================================================================
# SEQUENCE FORMATION FOR DATASET
#=======================================================================


# Since lstms work the best with longer input sequence, we consider 18 previous locations to predict the next position. Hence the sequence will be constructed of length 19 which includes input and the output

num_prev = 19
train_datalist = []
train_index_list = set(train_data['per_index'].to_list())
train_len_index = len(train_index_list)
long_index_list_train = []
long_index_list_test = []

for count, temp_index in enumerate(train_index_list):
    temp_data = train_data[train_data['per_index'] == temp_index]
    len_cluster = len(temp_data)
    if len_cluster >= 30:
    	long_index_list_train.append(temp_index)
    if len_cluster >= num_prev:
        for i in range(len_cluster-num_prev):
            train_datalist.append([temp_data.iloc[i:i+num_prev,3].to_list(), temp_data.iloc[i:i+num_prev,4].to_list()])

print('Training datalist has been compiled')

test_datalist = []
test_index_list = set(test_data['per_index'].to_list())
test_len_index = len(test_index_list)

for count, temp_index in enumerate(test_index_list):
    temp_data = test_data[test_data['per_index'] == temp_index]
    len_cluster = len(temp_data)
    
    if len_cluster >= 30:
    	long_index_list_test.append(temp_index)
    if len_cluster >= num_prev:
        for i in range(len_cluster-num_prev):
            test_datalist.append([temp_data.iloc[i:i+num_prev,3].to_list(), temp_data.iloc[i:i+num_prev,4].to_list()])

print('Testing datalist has been compiled')
...
datalist_close = []
for index in long_index_list_train:
    temp_data = train_data[train_data['per_index'] == index]
    datalist_close.append([temp_data.iloc[:30,4].to_list(),temp_data.iloc[:30,5].to_list()])

for index in long_index_list_test:
	temp_data = test_data[test_data['per_index'] == index]
	datalist_close.append([temp_data.iloc[:30,4].to_list(),temp_data.iloc[:30,5].to_list()])
        
final_data_close = torch.transpose(torch.from_numpy(np.array(datalist_close)).float(),0,2)
...
class FC_big_model(nn.Module):
    ...
    def __init__(self, input_dim):
        ...
        self.fc1 = nn.Linear(in_features = input_dim, out_features = 90, bias = True)
        self.relu1 = nn.GELU()
        self.fc2 = nn.Linear(in_features = 90, out_features = 22, bias = True)
        self.relu2 = nn.GELU()
        self.fc3 = nn.Linear(in_features = 22, out_features = 2, bias = True)
...
class FC_small_model(nn.Module):
    ...
    def __init__(self, input_dim):
        ...
        self.fc1 = nn.Linear(in_features = input_dim, out_features = 5, bias = True)
        self.relu1 = nn.GELU()
...
for ensemble_index in range(num_ensembles):
	...
	tr_data = rearrange(train_data, 'coords samples steps -> samples steps coords')
	te_data = rearrange(test_data, 'coords samples steps -> samples steps coords')
	...
	tr_data = tr_data[torch.randperm(tr_data.size()[0])]
	te_data = te_data[torch.randperm(te_data.size()[0])]

	dl = DataLoader(tr_data, batch_size = 30, shuffle = True)
	vdl = DataLoader(te_data, batch_size = 30, shuffle = True)

	#=======================================================================
	# Model details
	#=======================================================================
	
	#model = forecasterModel(2,50,n_layers = 1, do = 0.1)#.to(device)
	model1 = np_lstm_big(2,20,n_layers = 2, do = 0.1)
	model2 = FC_big_model(36)
	...
	for e in range(epochs):
		...
		for sample in dl:
		    ...
		    #loss = torch.sqrt(loss_fn(prediction,y))
		    ls2 += loss.item()
		    loss.backward()
		    optim2.step()
	...
	torch.save(error1_tensor,'/mnt/qb/work/goswami/ranganathabr08/common_directory/Feb_results/errors_lstm_big_model_'+str(ensemble_index)+'.pt')
	torch.save(model1, '/mnt/qb/work/goswami/ranganathabr08/common_directory/Feb_results/lstm_big_model_'+str(ensemble_index)+'.pt')
	...
	torch.save(error2_tensor,'/mnt/qb/work/goswami/ranganathabr08/common_directory/Feb_results/errors_fc_big_model_'+str(ensemble_index)+'.pt')
	torch.save(model2, '/mnt/qb/work/goswami/ranganathabr08/common_directory/Feb_results/fc_big_model_'+str(ensemble_index)+'.pt')
	...
	torch.save(ls_full_tensor, '/mnt/qb/work/goswami/ranganathabr08/common_directory/Feb_results/close_open_error_lstm'+str(ensemble_index)+'.pt')
	
	new_final_data = copy.deepcopy(rearrange(final_data_close, 'samples seq coords -> samples coords seq'))

	x0 = new_final_data[:,:,:18]
	ls = []
	ls_close = []

	for i in range(30-18):
	
		x = new_final_data[:,:,i:i+18]
		x_flat = torch.concat((x[:,0], x[:,1]), axis = 1)
		y = new_final_data[:,:,i+18]
		x0_flat = torch.concat((x0[:,0], x0[:,1]), axis = 1)
		prediction_close = model2.predict(x0_flat)
		prediction = model2.predict(x_flat)
		x0 = torch.concat((x0[:,:,1:], prediction.view(-1,2,1)), axis = 2)
		loss_close = haversine(prediction_close, new_final_data[:,:,i+18])
		loss = haversine(prediction, new_final_data[:,:,i+18])
		ls_close.append(loss_close.item())
		ls.append(loss.item())
	...
	ls_close_tensor = torch.Tensor(ls_close).view(-1,1)
	ls_full_tensor = torch.concat((ls_tensor, ls_close_tensor),axis = 1)
	torch.save(ls_full_tensor, '/mnt/qb/work/goswami/ranganathabr08/common_directory/Feb_results/close_open_error_fc'+str(ensemble_index)+'.pt')
